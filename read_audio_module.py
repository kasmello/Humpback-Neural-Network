...
def wav_to_spec():
    ...
    for location in selection_table_locations:
        ...
        for file in files:
            ...
            if len(dt) == 0 or filename[0:2] == '._':
                continue
            wavstring = ''
            clean_wavform = None
            wavform = None
            sample_rate = 0
            progress_table[filename] = progress_table.get(filename,0)
            for row in tqdm(range(len(dt))):
                wav_dir = dt.iloc[row]['Begin Path']
                start = dt.iloc[row]['Beg File Samp (samples)']
                dur = dt.iloc[row]['Sample Length (samples)']
                label = dt.iloc[row]['Code']
                label = switch_dictionary.get(label,str(label))
